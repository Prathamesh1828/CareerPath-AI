...
export function NotificationManagement() {
  ...
  const clearSpamTimers = () => {
    ...
    // Also clear the in-memory trigger tracker
    triggeredNotifications.current.clear();

    toast({
      title: "Spam Protection Reset",
      description:
        "Notification cooldowns have been cleared. You may receive new notifications immediately.",
    });
  };

  const testCourseNotification = () => {
    const notificationKey = `course-8-your_updated_skills-${Date.now()}`;

    // Check if this notification was recently triggered
    if (triggeredNotifications.current.has(notificationKey)) {
      toast({
        title: "Duplicate Prevented",
        description: "This notification was already triggered recently.",
        variant: "destructive",
      });
      return;
    }
    ...
    // Remove from tracker after 5 seconds to allow retesting
    setTimeout(() => {
      triggeredNotifications.current.delete(notificationKey);
    }, 5000);
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 mt-1">
                We automatically prevent duplicate notifications with cooldown
                periods:
              </p>
        ...
        <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
          <strong>Tip:</strong> If you're receiving too many notifications, you
          can:
          <br />• Adjust notification preferences in the settings above
          <br />• Clear all notifications to start fresh
          <br />• Reset cooldowns if you want to receive new notifications
          immediately
        </div>
      </CardContent>
    </Card>
